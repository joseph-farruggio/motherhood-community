...
function mhc22_enqueue_scripts() {
	...
	wp_enqueue_style( 'mhc22', mhc22_asset('css/app.css'), array(), $theme->get('Version').'1' );

	wp_enqueue_script( 'splide', mhc22_asset('js/splide.min.js'), array(), '4.0.7', true );
	wp_enqueue_script( 'mhc22', mhc22_asset('js/app.js'), array(), $theme->get('Version').'1', true );
}
...
function mhc22_defer_parsing_of_js( $url ) {
    ...
    if ( strpos( $url, 'wrapper.min.js' ) ) return $url;
